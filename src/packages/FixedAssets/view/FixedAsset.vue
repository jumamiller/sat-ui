      ...
      <v-card-text>
        ...
        >
          ...
          <template v-slot:item.assigned_to="{item}">
            <span>{{item.assigned_to && item.assigned_to.user.first_name || 'N/A'}} {{item.assigned_to && item.assigned_to.user.last_name}}</span>
          </template>
          <!--end assigned_to-->
          <!--start location-->
...
export default {
  ...
  beforeRouteEnter(to,from,next){
    next(v=>{
      v.$store.dispatch('FixedAsset/getAssets');
    })
  },
  data: () => ({
    dialog: false,
  ...
  computed:{
    assets(){
      return this.$store.getters['FixedAsset/FixedAssetGetter']("assets")
    }
  },
  methods: {
    //
    ...
    redirectToCardInfo(code){
      this.$router.push({name:"FixedAssetCard",params:{code}})
    },
  }
}
</script>
